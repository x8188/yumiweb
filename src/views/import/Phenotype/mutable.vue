    ...
    <el-row :gutter="10" class="mb8">
      <el-col :span="1.5">
        <el-button
          ...
          size="mini"
          @click="handleAdd"
          v-hasPermi="['zeamap:ImportMutable:add']"
        >新增</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          :disabled="single"
          @click="handleUpdate"
          v-hasPermi="['zeamap:ImportMutable:edit']"
        >修改</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          :disabled="multiple"
          @click="handleDelete"
          v-hasPermi="['zeamap:ImportMutable:remove']"
        >删除</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          size="mini"
          @click="handleExport"
          v-hasPermi="['zeamap:ImportMutable:export']"
        >导出</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
...
<script>
import { listImportMutable, getImportMutable, delImportMutable, addImportMutable, updateImportMutable } from "@/api/import/Phenotype/mutable";
...
export default {
  ...
  methods: {
    ...
    getList() {
      this.loading = true;
      listImportMutable(this.queryParams).then(response => {
        this.ImportMutableList = response.rows;
    ...
    handleSelectionChange(selection) {
      this.ids = selection.map(item => item.mutableId)
    ...
    handleAdd() {
      this.reset();
      this.open = true;
      this.title = "添加ImportMutable";
    },
    /** 修改按钮操作 */
    handleUpdate(row) {
      this.reset();
      const mutableId = row.mutableId || this.ids
      getImportMutable(mutableId).then(response => {
        this.form = response.data;
        this.open = true;
        this.title = "修改ImportMutable";
      });
    },
    /** 提交按钮 */
    submitForm() {
      this.$refs["form"].validate(valid => {
        if (valid) {
          if (this.form.mutableId != null) {
            updateImportMutable(this.form).then(response => {
              this.$modal.msgSuccess("修改成功");
              this.open = false;
              this.getList();
            });
          } else {
            addImportMutable(this.form).then(response => {
    ...
    /** 删除按钮操作 */
    handleDelete(row) {
      const mutableIds = row.mutableId || this.ids;
      this.$modal.confirm('是否确认删除ImportMutable编号为"' + mutableIds + '"的数据项？').then(function() {
        return delImportMutable(mutableIds);
      }).then(() => {
        this.getList();
    ...
    /** 导出按钮操作 */
    handleExport() {
      this.download('zeamap/ImportMutable/export', {
        ...this.queryParams
      }, `ImportMutable_${new Date().getTime()}.xlsx`)
    },
    /** 导入按钮操作 */
    handleImport() {
