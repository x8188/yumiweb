<template>
  <div class="app-container">
    <el-form :model="queryParams" ref="queryForm" size="small" :inline="true" v-show="showSearch" label-width="320px">
      <el-form-item label="expression表关联的id" prop="uniquename">
        <el-input
          v-model="queryParams.uniquename"
          placeholder="请输入expression表关联的id"
          clearable
      ...
      <el-form-item label="Single expression viewer 的基因ID，查询字段" prop="commonname">
        <el-input
          ...
          placeholder="请输入Single expression viewer 的基因ID，查询字段"
          clearable
          @keyup.enter.native="handleQuery"
        />
      </el-form-item>
    ...
    <el-row :gutter="10" class="mb8">
      <el-col :span="1.5">
        <el-button
          ...
          size="mini"
          @click="handleAdd"
          v-hasPermi="['zeamap:Importfeature:add']"
        >新增</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          :disabled="single"
          @click="handleUpdate"
          v-hasPermi="['zeamap:Importfeature:edit']"
        >修改</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          :disabled="multiple"
          @click="handleDelete"
          v-hasPermi="['zeamap:Importfeature:remove']"
        >删除</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          size="mini"
          @click="handleExport"
          v-hasPermi="['zeamap:Importfeature:export']"
        >导出</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
...
<script>
import { listImportfeature, getImportfeature, delImportfeature, addImportfeature, updateImportfeature } from "@/api/import/Genomics/feature";
...
export default {
  ...
  data() {
    return {
      ...
      rules: {
        commonname: [
          { required: true, message: "Single expression viewer 的基因ID，查询字段不能为空", trigger: "blur" }
        ],
      }
    };
  },
  ...
  methods: {
    ...
    getList() {
      this.loading = true;
      listImportfeature(this.queryParams).then(response => {
        this.ImportfeatureList = response.rows;
    ...
    handleAdd() {
      this.reset();
      this.open = true;
      this.title = "添加Importfeature";
    },
    /** 修改按钮操作 */
    handleUpdate(row) {
      this.reset();
      const featureId = row.featureId || this.ids
      getImportfeature(featureId).then(response => {
        this.form = response.data;
        this.open = true;
        this.title = "修改Importfeature";
      });
    },
    /** 提交按钮 */
    submitForm() {
      this.$refs["form"].validate(valid => {
        if (valid) {
          if (this.form.featureId != null) {
            updateImportfeature(this.form).then(response => {
              this.$modal.msgSuccess("修改成功");
              this.open = false;
              this.getList();
            });
          } else {
            addImportfeature(this.form).then(response => {
    ...
    handleDelete(row) {
      ...
      this.$modal.confirm('是否确认删除Importfeature编号为"' + featureIds + '"的数据项？').then(function() {
        return delImportfeature(featureIds);
      }).then(() => {
        this.getList();
    ...
    /** 导出按钮操作 */
    handleExport() {
      this.download('zeamap/Importfeature/export', {
        ...this.queryParams
      }, `Importfeature_${new Date().getTime()}.xlsx`)
    },
    /** 导入按钮操作 */
    handleImport() {
