...
<script>
import { listCv, getCv, delCv, addCv, updateCv } from "@/api/import/Genomics/cv";
...
export default {
  ...
  methods: {
    ...
    getList() {
      this.loading = true;
      listCv(this.queryParams).then(response => {
    ...
    handleSelectionChange(selection) {
      this.ids = selection.map(item => item.cvId)
    ...
    handleAdd() {
      this.reset();
      this.open = true;
      this.title = "添加ImportCv";
    },
    /** 修改按钮操作 */
    handleUpdate(row) {
      this.reset();
      const cvId = row.cvId || this.ids
      getCv(cvId).then(response => {
        this.form = response.data;
        this.open = true;
        this.title = "修改ImportCv";
      });
    },
    /** 提交按钮 */
    submitForm() {
      this.$refs["form"].validate(valid => {
        if (valid) {
          if (this.form.cvId != null) {
            updateCv(this.form).then(response => {
              this.$modal.msgSuccess("修改成功");
              this.open = false;
              this.getList();
            });
          } else {
            addCv(this.form).then(response => {
    ...
    /** 删除按钮操作 */
    handleDelete(row) {
      const cvIds = row.cvId || this.ids;
      this.$modal.confirm('是否确认删除ImportCv编号为"' + cvIds + '"的数据项？').then(function() {
        return delCv(cvIds);
      }).then(() => {
        this.getList();
    ...
    /** 导出按钮操作 */
    handleExport() {
      this.download('zeamap/cv/export', {
        ...this.queryParams
      }, `cv_${new Date().getTime()}.xlsx`)
    }
  }
};
</script>
